...
class WhisperFeatureExtractor(SequenceFeatureExtractor):
    ...
    # XXX: start from torch.Tensor for a differentiable version
    def _torch_extract_fbank_features(self, waveform: torch.Tensor, device: str = "cpu"):
        """
        Compute the log-mel spectrogram of the audio using PyTorch's GPU-accelerated STFT implementation with batching,
        yielding results similar to cpu computing with 1e-5 tolerance.
        """
        ...
        mel_filters = torch.from_numpy(self.mel_filters).type(torch.float32).to("cuda")
        # if device != "cpu":
        #     mel_filters = mel_filters.to(device)
        mel_spec = mel_filters.T @ magnitudes
    ...
    # XXX: differentiable version of feature extraction in Qwen
    def __call__(
            self,
        ...
        """
        Main method to featurize and prepare for the model one or several sequence(s). Implementation uses PyTorch for
        all computations to ensure differentiability with respect to the input raw_speech.

        Args:
            raw_speech (torch.Tensor or List[torch.Tensor]):
                The sequence or batch of sequences to be padded. Each sequence must be a PyTorch tensor.
            truncation (bool, optional, defaults to True):
                Activates truncation to cut input sequences longer than max_length to max_length.
            pad_to_multiple_of (int, optional, defaults to None):
                If set, will pad the sequence to a multiple of the provided value.
            return_tensors (str, optional, defaults to 'pt'):
                Specifies the return type of the tensors.
            return_attention_mask (bool, optional):
            ...
            sampling_rate (int, optional):
                The sampling rate of raw_speech.
            do_normalize (bool, optional, defaults to False):
