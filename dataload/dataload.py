...
def get_audio_list(dataset, jailbreak, voice_id=0, speed=1):
    ...
    if dataset == 'advbench':
        if jailbreak == "text_gcg" or jailbreak == "none" or jailbreak == "audio_ours" or jailbreak == "speechgptaudio" or jailbreak == "blackbox" or jailbreak == "audio_ours_universal":
            input_file = 'data/text_jailbreak/gcg_advbench_behaviour.jsonl'
        elif jailbreak == "text_autodan":
            input_file = 'data/text_jailbreak/autodan_advbench_behaviour.jsonl'
        elif jailbreak == "text_beast":
            input_file = 'data/text_jailbreak/beast_advbench_behaviour.jsonl'
        else:
            input_file = 'data/text_jailbreak/gcg_advbench_behaviour.jsonl'
    ...
    audio_dir = f'data/audio/{dataset}/{jailbreak}/{voice_id}/{speed}'
    ...
    for id, instance in tqdm(enumerate(text_data)):
        ...
        if jailbreak == "speechgptaudio":
            audio_ori = os.path.join(f'data/audio/{dataset}/none/{voice_id}/{speed}', f"{id}.wav")
            audio_suffix = os.path.join(f'output/{dataset}/audio_ours/speechgpt/0.8', f"jailbreak_audio_{id}.wav")
            audio_list.append([[audio_ori, audio_suffix]])
            # audio_list.append([audio_ori])
        elif jailbreak == "qwen_transfer":
            audio_jailbreak = os.path.join(f'output/{dataset}/audio_ours/qwen2/maintable',f"{id}.wav")
            audio_list.append([audio_jailbreak])
        elif jailbreak == "llama_omni_transfer":
            audio_jailbreak = os.path.join(f'output/{dataset}/audio_ours/omni_speech/maintable',f"{id}.wav")
            audio_list.append([audio_jailbreak])
        else:
            audio_file = os.path.join(f'data/audio/{dataset}/{jailbreak}/{voice_id}/{speed}', f"{id}.wav")
